...
func test_start_begins_at_entry_title_and_presents_first_line() -> void:
	...
	var started: bool = controller.start(compiled, "start", context, presenter)
	assert_true(started)
...
func test_start_returns_false_when_conversation_already_active() -> void:
	var compiled: CompiledDialogue = _compile_fixture()
	var context: GameContext = _mock_context()
	var presenter: IDialoguePresenter = _mock_presenter()
	var controller: Node = _new_controller()
...
func test_start_emits_step_ready_signal() -> void:
	var compiled: CompiledDialogue = _compile_fixture()
	var context: GameContext = _mock_context()
	var presenter: IDialoguePresenter = _mock_presenter()
	var controller: Node = _new_controller()
